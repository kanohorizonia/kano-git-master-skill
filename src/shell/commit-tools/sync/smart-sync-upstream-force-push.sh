...
discover_workspace_repos() {
  ...
  repos_json="$("$scripts_root/core/discover-repos.sh" --root "$root" --format json --include-types root,registered,unregistered --metadata-level minimal 2>/dev/null || true)"
