// Commit command implementation utilities and CLI callback wiring.
    ...
    cmd->callback([=]() {
        ...
        for (const auto& repo : repoList) {
            ...
            if (effectiveMessage.empty() && !stageMessages.empty()) {
                workspace::RepoRecord repoRecord;
                repoRecord.path = repo;
                const auto planned = BuildRepoCommitRunbooks(
                    {repoRecord}, stageMessages, workspaceRoot, "", false);
                if (!planned.empty() && !planned.front().commits.empty()) {
                    effectiveMessage = Trim(planned.front().commits.front().message);
                }
            }
