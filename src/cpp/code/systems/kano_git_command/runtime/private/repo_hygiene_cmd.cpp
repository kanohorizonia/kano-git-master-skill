...
void FixRepoHygieneRecursive(const std::filesystem::path& InWorkspaceRoot) {
    CheckRepoHygiene(InWorkspaceRoot, true, true, true);
}
...
void RegisterRepoHygiene(CLI::App& InApp) {
    ...
    cmd->add_flag("-r,--recursive", *recursive, "Apply to all submodules recursively");
    ...
    check->callback([=]() {
        ...
        CheckRepoHygiene(root, false, *checkArchiveSafe, *recursive);
    });
    ...
    fix->callback([=]() {
        const auto root = std::filesystem::weakly_canonical(std::filesystem::path(*repo));
        CheckRepoHygiene(root, true, *fixArchiveSafe, *recursive);
    });
}
