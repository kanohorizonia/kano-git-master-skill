...
TEST_CASE("exact-path dry-run reports included and excluded paths without mutation",
          "[functional][KG-TSK-0112][dry-run]") {
    ...
    const auto result = RunKog({
        "commit", "--exact-path", "selected.txt", "-m", "[Test][Chore] preview", "--dry-run",
    }, repo);
