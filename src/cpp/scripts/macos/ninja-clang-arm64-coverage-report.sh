#!/usr/bin/env bash
# =============================================================================
# macOS ARM64 Coverage - Generate HTML Report
# =============================================================================
# Uses llvm-cov (Xcode) to generate HTML coverage report for ARM64.
#
# Usage:
#   bash ninja-clang-arm64-coverage-report.sh
#
# Environment:
...
profdata_file="$KOG_COVERAGE_ROOT/merged-arm64.profdata"
html_dir="$KOG_COVERAGE_ROOT/html-arm64"
...
# Merge profraw files
profraw_file="$profraw_dir/macos-arm64.profraw"
if [[ ! -f "$profraw_file" ]]; then
    echo "[ERROR] Profraw file not found: $profraw_file" >&2
    echo "[ERROR] Run ninja-clang-arm64-coverage-run.sh first." >&2
    exit 1
fi

echo "[coverage-report-macos-arm64] Merging coverage data..."
"$llvm_profdata" merge "$profraw_file" -o "$profdata_file" 2>&1

# Generate HTML report
echo "[coverage-report-macos-arm64] Generating HTML report..."
"$llvm_cov" show \
    "$binary_path" \
...
# Text summary
echo ""
echo "[coverage-report-macos-arm64] Text summary:"
"$llvm_cov" report \
    "$binary_path" \
    -instr-profile="$profdata_file" \
...
echo ""
echo "[coverage-report-macos-arm64] Reports:"
