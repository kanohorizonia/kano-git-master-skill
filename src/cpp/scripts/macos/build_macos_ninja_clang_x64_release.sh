...
resolve_macos_compiler_flags() {
	...
	out_ref=(
		...
		-DKOG_PRESET_NAME=macos-make-clang-x64
	)
	...
	if [[ "${KOG_BUILD_USE_LLVM:-0}" == "1" ]]; then
		...
		out_ref=(
			...
			-DKOG_PRESET_NAME=macos-make-llvm-x64
		)
		if [[ -n "$sdk_path" ]]; then
...
if command -v ninja >/dev/null 2>&1; then
	kog_run_unix_preset "macos-ninja-clang-x64" "macos-ninja-clang-x64-release"
	exit 0
fi

echo "ninja not found. Falling back to Unix Makefiles for macOS x64 release build." >&2
compiler_flags=()
...
(
	...
	cmake -S . -B out/obj/macos-make-clang-x64 \
		-G "Unix Makefiles" \
		-DCMAKE_BUILD_TYPE=Release \
		-DCMAKE_OSX_ARCHITECTURES=x86_64 \
		"${compiler_flags[@]}" \
		-DKOG_ENABLE_MODULES="$modules_value"
	cmake --build out/obj/macos-make-clang-x64 --parallel
)
