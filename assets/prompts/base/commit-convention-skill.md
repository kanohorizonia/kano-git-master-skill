# Kano Commit Convention (KCC) — KCC-STCC Profile

## Format

```
[<Subsystem>][<Type>] <Summary>
```

Optional breaking change flag:
```
[<Subsystem>][<Type>][Breaking] <Summary>
```

## Examples

```
[UGS][BugFix] Retry tagged output parsing on proxy glitch
[Build][Chore] Update CI bootstrap script
[UI][Feature] Add pooled compass marker widgets
[Core][Refactor] Extract shared path resolver
[Plan][BugFix] Fix UTF-8 encoding garble in LogInvocation header
[Test][Feature] Add bug condition and preservation property tests
```

## Field Rules

### Subsystem (required, open set)
- Length: 2–24 characters, alphanumeric only, first char must be a letter
- No spaces, underscores, dots, or hyphens
- Recommended PascalCase (e.g. `Build`, `Core`, `Plan`, `Commit`, `Test`, `Infra`)
- No registry — lint must NOT reject unknown subsystem names

### Type (required, closed set)
Allowed values: `Feature`, `BugFix`, `Refactor`, `Perf`, `Chore`, `Test`, `Docs`

### Summary (required)
- Start with an English verb (Add / Fix / Update / Remove / Refactor)
- Keep to ~50–72 characters

## Validation Regex

```
^\[[A-Za-z][A-Za-z0-9]{1,23}\]\[(Feature|BugFix|Refactor|Perf|Chore|Test|Docs)\](\[(Breaking)\])? .+$
```

## Important

- Do NOT use Conventional Commits format (`feat:`, `fix:`, `chore:`) — use KCC `[Subsystem][Type]` format
- If the diff includes `[context] worktree is clean; using HEAD patch for amend/reword.`, ignore it. Do NOT use `Context` as the subsystem or write summaries about the worktree being clean. Focus entirely on the actual code changes.
