...
- While preparing the native payload binary in this worktree, the build exposed missing dependency materialization for the then-current C++ setup; at that time the local build still depended on `cli11` / `ftxui` submodules plus the ignore-source mirror before the later fetch-first migration.
